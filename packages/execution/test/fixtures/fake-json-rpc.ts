...
export function fakeJsonRpc(request: JsonRpcRequest): JsonRpcResponse {
  switch (request.method) {
    case 'eth_chainId':
      return { jsonrpc: '2.0', id: request.id, result: fixtures.eth_chainId };
    case 'eth_getBalance':
      if (
    ...
    case 'eth_getBlockByNumber':
      return {
        jsonrpc: '2.0',
        id: request.id,
        result: fixtures.eth_getBlockByNumber,
      };
    default:
...
export const fakeFetchImplThatCanOnlyDoNetworkDetection = async (
  ...
  const networkDetectionRequests = requests.filter(
    (request: JsonRpcRequest) => request.method === 'eth_chainId',
  );

  if (networkDetectionRequests.length < requests.length) {
    throw new Error('Some error');
  }

  return networkDetectionRequests.map(fakeJsonRpc);
};
    ...
    return requests.map((request: JsonRpcRequest) => {
      if (request.method === 'eth_chainId') {
        return {
          jsonrpc: '2.0',
          id: request.id,
          result: BigNumber.from(chainId).toHexString(),
        };
      }
