import { rangePromise } from '../src';

describe('rangePromise', () => {
  const call = async <T>(v: T): Promise<T> => v;

  it('should work for one batch', async () => {
    await expect(rangePromise(call, 1, 2, 1)).resolves.toEqual([1, 2]);
    await expect(rangePromise(call, 0, 3, 1)).resolves.toEqual([0, 1, 2, 3]);
    await expect(rangePromise(call, -5, -3, 1)).resolves.toEqual([-5, -4, -3]);
    await expect(rangePromise(call, -2, 1, 1)).resolves.toEqual([-2, -1, 0, 1]);
  });

  it('should work for two batches', async () => {
    await expect(rangePromise(call, 0, 3, 2)).resolves.toEqual([0, 1, 2, 3]);
    await expect(rangePromise(call, -3, -1, 2)).resolves.toEqual([-3, -2, -1]);
  });

  it('should work for one value', async () => {
    await expect(rangePromise(call, 1, 1, 1)).resolves.toEqual([1]);
    await expect(rangePromise(call, 3, 3, 1)).resolves.toEqual([3]);
  });

  it('should work for one value and multiply batches', async () => {
    await expect(rangePromise(call, 1, 1, 5)).resolves.toEqual([1]);
    await expect(rangePromise(call, 3, 3, 5)).resolves.toEqual([3]);
  });

  it('should work if batch size is greater than step', async () => {
    await expect(rangePromise(call, 0, 1, 3)).resolves.toEqual([0, 1]);
    await expect(rangePromise(call, 2, 5, 7)).resolves.toEqual([2, 3, 4, 5]);
  });
